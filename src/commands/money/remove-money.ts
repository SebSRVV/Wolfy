...
const removeMoney = async (client: Client, interaction: ChatInputCommandInteraction) => {
	try {
		...
		if (monto <= 0)
			return interaction.reply({
				content: "El monto a remover debe ser mayor a 0.",
				ephemeral: true
			});

		member.money.economy.push({
			amount: -monto,
			reason: "Dinero removido por un administrador",
			date: new Date()
		});
		...
		interaction.reply({
			content: `Se removieron $${monto} a la cuenta de <@${miembro}>.`,
			ephemeral: true
		});
	} catch (error) {
		console.error(error);
