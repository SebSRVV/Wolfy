...
export const command: CommandInterface = {
	...
	async run(client, interaction) {
		try {
			...
			if (!embed) return interaction.reply({ content: "No se pudo cambiar la prioridad.", ephemeral: true });

			const notifyChannel = await client.channels
				.fetch("1287980502686367796")
				.then(c => c)
				.catch(() => null);
			if (!notifyChannel) return interaction.reply({ content: "No se pudo cambiar la prioridad.", ephemeral: true });

			if (notifyChannel.type !== ChannelType.GuildText)
				return interaction.reply({ content: "No se pudo cambiar la prioridad.", ephemeral: true });
			...
			interaction.reply({
				content: "Prioridad cambiada exitosamente.",
				ephemeral: true
			});
		} catch (error) {
			console.error(error);
